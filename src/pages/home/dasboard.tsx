        ...
        const handleFileUpload = (e: ChangeEvent<HTMLInputElement>, type: 'photo' | 'video') => {
        ...
        return (
            ...
            <h1 className="text-2xl text-center mb-6  font-bold mb-4">Business Dashboard</h1>
            ...
            <div className="bg-white shadow rounded-lg p-4">
                ...
                {businessInfo.media.map((media, index) => (
                    ...
                    {media.type === 'photo' ? (
                        <img src={media.preview} alt={`Business`} className="w-full h-32 object-cover rounded" />
                    ) : (
                        <video src={media.preview} className="w-full h-32 object-cover rounded" />
                    )}
                    <button onClick={() => handleRemoveMedia(index)} className="absolute top-0 right-0 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center m-1">X</button>
                    </div>
                ))}
                </div>
            ...
            <div className="bg-white shadow rounded-lg mb-4 p-4">
                ...
                {['name', 'category', 'address', 'phoneNumber', 'websiteUrl'].map((field) => (
                    <div key={field}>
                    ...
                    {editing === field ? (
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleInputChange(field as keyof BusinessInfo, e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        />
                    ) : (
                        <p className="mt-1">{businessInfo[field as keyof BusinessInfo] as string}</p>
                    )}
                    <div className="mt-2">
                        {editing === field ? (
                        <button onClick={handleSave} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Save</button>
                        ) : (
                        <button onClick={() => handleEdit(field)} className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Edit</button>
                        )}
                        <button onClick={() => handleDelete(field as keyof BusinessInfo)} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ml-2">Delete</button>
                    </div>
                    </div>
                ))}
                </div>
            ...
            <div className="bg-white shadow rounded-lg mb-4 p-4">
                ...
                {Object.entries(businessInfo.businessHours).map(([day, hours]) => (
                    <div key={day}>
                    <label className="block text-sm font-medium text-gray-700">{day.charAt(0).toUpperCase() + day.slice(1)}</label>
                    <div className="flex space-x-2 mt-1">
                        <input
                        type="time"
                        value={hours.open}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleHoursChange(day, 'open', e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        />
                        <input
                        type="time"
                        value={hours.close}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => handleHoursChange(day, 'close', e.target.value)}
                        className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                        />
                    </div>
                    </div>
            ...
            <div className="bg-white shadow rounded-lg mb-4 p-4">
                ...
                {editing === 'description' ? (
                    ...
                    <button onClick={() => handleEdit('description')} className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Edit</button>
                )}
                <button onClick={() => handleDelete('description')} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded ml-2">Delete</button>
                </div>
            </div>
